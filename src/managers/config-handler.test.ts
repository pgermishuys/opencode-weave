...
describe('ConfigHandler', () => {
  ...
  it('returns empty MCPs and builtin commands', async () => {
    ...
    expect(Object.keys(result.commands)).toHaveLength(
  ...
  it('returns MCPs when mcp config is provided with enabled servers', async () => {
    const handler = new ConfigHandler({
      pluginConfig: {
        mcp: {
          enabled: {
    ...
    expect(result.mcps.websearch).toBeDefined();
    expect(result.mcps.context7).toBeDefined();
    expect(result.mcps.grep_app).toBeUndefined();
  });

  it('filters MCPs by disabled_mcps list', async () => {
    const handler = new ConfigHandler({
      pluginConfig: {
        mcp: {
          enabled: {
    ...
    expect(Object.keys(result.mcps)).toHaveLength(2);
    expect(result.mcps.websearch).toBeDefined();
    expect(result.mcps.grep_app).toBeDefined();
    expect(result.mcps.context7).toBeUndefined();
  });

  it('includes custom MCP servers from config', async () => {
    const handler = new ConfigHandler({
      pluginConfig: {
        mcp: {
          enabled: {
    ...
    expect(Object.keys(result.mcps)).toHaveLength(2);
    expect(result.mcps.websearch).toBeDefined();
    expect(result.mcps.custom_server).toBeDefined();
  });
});
