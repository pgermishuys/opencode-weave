...
describe("plan completion on edit", () => {
  it("tool.execute.after calls promptAsync when plan edit completes all tasks", async () => {
    ...
    const hooks = makeHooks({
      workContinuation: (_sessionId: string) => ({
        continuationPrompt: "All 3 tasks complete. Run post-execution review.",
        targetAgent: "Loom (Main Orchestrator)",
      }),
    })
    ...
    expect(promptAsyncCalls.length).toBe(1)
    expect(promptAsyncCalls[0].path.id).toBe("s1")
    expect(promptAsyncCalls[0].body.agent).toBe("Loom (Main Orchestrator)")
    expect(promptAsyncCalls[0].body.parts[0].text).toContain("All 3 tasks complete")
  })

  it("tool.execute.after does NOT call promptAsync when plan has remaining tasks", async () => {
    ...
    const hooks = makeHooks({
      workContinuation: (_sessionId: string) => ({
        continuationPrompt: "Continue working. 2/5 tasks done.",
        // No targetAgent — plan is not complete
      }),
    })
  ...
  it("tool.execute.after does NOT call promptAsync for non-plan file edits", async () => {
    const promptAsyncCalls: unknown[] = []
    ...
    const hooks = makeHooks({
      workContinuation: (_sessionId: string) => ({
        continuationPrompt: "All tasks complete.",
        targetAgent: "Loom (Main Orchestrator)",
      }),
    })
  ...
  it("tool.execute.after does not crash when client is absent and plan is complete", async () => {
    const hooks = makeHooks({
      workContinuation: (_sessionId: string) => ({
        continuationPrompt: "All tasks complete.",
        targetAgent: "Loom (Main Orchestrator)",
  ...
  it("tool.execute.after does not crash when promptAsync throws", async () => {
    const mockClient = {
      session: {
        promptAsync: async () => {
